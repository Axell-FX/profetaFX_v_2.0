...
def show_forecast(m, forecast, data, p, df_train, currency, c2, selected_stock):
    ...
    only_forecast["Accuracy (%)"] = (prediction / original / original_2) * 100
    ...
    linear = LinearRegression().fit(original_2, prediction)
    rmpse = (
        np.sqrt(np.nanmean(np.square(((original - original_2 - prediction) / original)))) * 100
    ) ** 2
